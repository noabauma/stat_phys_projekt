...
bool MDRun::reactionDetected(const std::vector<double>& x, const std::vector<int>& elements){
  return true;
}
void MDRun::run(std::vector<double> &x, std::vector<double> &v, std::vector<int> &elements) {
    ...
    for (int nstep = 0; nstep < par.numberMDSteps; nstep++) {
        time += par.timeStep;
        performStep(x, v, nstep, time, elements);
    }
    reactionDetected(x, elements);
    printAverages(time);
}
...
void MDRun::performStep(std::vector<double>& positions, std::vector<double>& velocities,
  int nstep, double time, std::vector<int>& elements) {
     ...
     */
    forceCalculator.calculate(positions, forces, elements);
    ...
    for (int j3 = 0; j3 < nat3; j3++) {
        ...
        double newVelocity;
        if((j3>=6 && j3<=17)||(j3>=30 && j3<=35)){
          newVelocity = (oldVelocity+forces[j3]*dtmC)*scal;
          oldKineticEnergy += 12*(newVelocity * newVelocity)/2.;
          newKineticEnergy += 12*(oldVelocity + newVelocity) * (oldVelocity + newVelocity)/8.;
        }
        else{
          newVelocity = (oldVelocity+forces[j3]*dtmH)*scal;
          oldKineticEnergy += (newVelocity * newVelocity)/2.;
          newKineticEnergy += (oldVelocity + newVelocity) * (oldVelocity + newVelocity)/8.;
        }
