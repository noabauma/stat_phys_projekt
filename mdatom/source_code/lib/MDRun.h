...
class MDRun {
  ...
  public:
    ...
    void run(std::vector<double> &x, std::vector<double> &v, std::vector<int> &elements);
  ...
  private:
    ...
    void performStep(std::vector<double>& positions, std::vector<double>& velocities, int nstep, double time, std::vector<int>& elements);
    ...
    bool reactionDetected(const std::vector<double>& pos, const std::vector<int>& elements);
